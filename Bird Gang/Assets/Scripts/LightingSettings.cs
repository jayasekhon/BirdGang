...
public class LightingSettings : MonoBehaviour
{
    ...
    void Update() 
    {
        if (nightTime) 
        {
            ...
            mainLight.color=Color.Lerp(directionalLight.color, new Color(0f, 0.09997659f, 0.4811321f, 1f), step);
            mainLight.intensity = Mathf.Lerp(1.2f, 0.6f, step);
            step += Time.deltaTime/6f;
